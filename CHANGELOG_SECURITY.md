...
### Level 9 — URL 特殊字符注入（链接截断）

| 项目 | 内容 |
|------|------|
| **漏洞** | 文件名含 `#` `?` `&` 等字符时，构造的 URL `/uploads/file#1.png` 中 `#1.png` 被浏览器当作锚点，图片无法加载 |
| **攻击场景** | 上传文件名为 `test#1.png`，预览链接截断为 `/uploads/test`，图片无法显示 |
| **修复** | URL 特殊字符替换为 `_`，URL 使用 `quote()` 编码 |

### Level 10 — 文件覆盖攻击（无警告覆盖）

| 项目 | 内容 |
|------|------|
| **漏洞** | 同一文件名上传两次，后者无声覆盖前者，可用于替换他人头像为恶意内容 |
| **修复** | 上传前检查文件是否存在，已存在则拒绝并提示 |

### Level 11 — 上传频率缺失（DOS 攻击）

| 项目 | 内容 |
|------|------|
| **漏洞** | 无上传频率限制，攻击者可短时间内上传大量大文件填满磁盘 |
| **修复** | Session 级别限制：每 60 秒最多 10 次上传 |
